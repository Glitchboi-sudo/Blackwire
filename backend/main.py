...
# Security: safe request/response ID pattern (hex digits only, as generated by hashlib.md5)
_SAFE_ID_RE = re.compile(r'^[a-zA-Z0-9_-]{1,64}$')
_BACKEND_DIR = Path(__file__).parent.resolve()
_ALLOWED_PROXY_MODES = {"regular", "upstream", "socks5", "transparent"}
# Mitmproxy flags that could be exploited to load external scripts or write files anywhere
_BLOCKED_MITM_FLAGS = {"--scripts", "-s", "--save-stream-file", "--save-stream-filter",
                        "--allow-hosts", "--ignore-hosts", "--ssl-insecure"}

def _validate_id(rid: str) -> bool:
    """Return True only if rid is a safe alphanumeric ID (no path separators or special chars)."""
    return bool(_SAFE_ID_RE.fullmatch(rid))

def _action_file(rid: str) -> Path:
    """Construct the action file path and verify it stays inside _BACKEND_DIR."""
    if not _validate_id(rid):
        raise ValueError(f"Unsafe request_id: {rid!r}")
    path = (_BACKEND_DIR / f".action_{rid}.json").resolve()
    if path.parent != _BACKEND_DIR:
        raise ValueError(f"Path traversal detected in request_id: {rid!r}")
    return path

def _action_resp_file(rid: str) -> Path:
    """Construct the response action file path and verify it stays inside _BACKEND_DIR."""
    if not _validate_id(rid):
        raise ValueError(f"Unsafe response_id: {rid!r}")
    path = (_BACKEND_DIR / f".action_resp_{rid}.json").resolve()
    if path.parent != _BACKEND_DIR:
